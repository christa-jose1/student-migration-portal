...
const Canadapage = () => {
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:5000/api/guides/guides/CA");
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const usData = response.data.find((item) => item.country === "C");
      ...
      {selectedCourse && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50">
          <motion.div
            ...
            className="bg-gray-900 rounded-lg p-6 w-96 border border-gray-700 shadow-xl relative max-h-[500px] overflow-y-auto"
          >
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="mt-4 w-full py-2 text-sm bg-gradient-to-r from-blue-600 to-purple-600 hover:shadow-lg rounded-lg transition-all"
            >
              Apply Now
            </motion.button>
          </motion.div>
